...
一つの設定ファイルで、LinuxやmacOSだけでなく、WindowsのVC++のビルドも記述できるので、マルチプラットフォームで共有ライブラリを作る今回のプロジェクトに相応しいと考えて、採用しています。
...
* Build Tools for Visual Studio 2017 のダウンロードボタンの場所が分かりにくいのですが、ページの最後の方の「ツール及びフレームワーク」のところにあります。
* COM相互運用のクラスライブラリjtalkCOMx86.dll または jtalkCOMx64.dllを作成するときは、Visual Studio のインストーラを使って C++/Cli サポートをインストールしておきます。
* 将来の2017とは違うバージョンのVisual Studioでもビルド可能か分かりませんが、そのときは後述のコマンドプロンプトを使った方法で試してください。
...
MSVCのみ。遅延署名。コマンドラインからのビルドでは通常無効。
...
Windowsの場合コマンドプロンプトで``c:\open_jtalk\bin``をカレントディレクトリにして実行するか、c:\open_jtalk\binフォルダ開いてマウスで対象をダブルクリックでもかまいません。
...
正常にビルドが完了すれば、ライブラリとインクルードファイルは所定の場所に配置されているはずです。
...
x86_64-w64-mingw32-gccでコンパイルするか、i686-w64-mingw32-gccでコンパイルするかで、
リンクするライブラリが違ってくるためです。
-Lオプションによるライブラリの明示的な指定を追加します。
...
です。
C++からの利用はFFIとは言えませんが、仲介するヘッダファイルとサンプルプログラムは、ffiに配置しています。
...
各言語における共有ライブラリの参照の仕方が分かりさえすれば、ここで用意していないプログラミング言語でも利用できるはずです。

## JTalkCOM を使ったプログラミング

JTalkCOMx86.dll と JTalkCOMx64.dll を使った Windowsでのプログラミングの方法を示します。
なお、リポジトリのビルドで生成された遅延署名のdllでは動きません。別途配布版を利用してください。
...
copy c:\open_jtalk\bin\%jtalkcom% .
csc /platform:%VSCMD_ARG_HOST_ARCH% /target:exe /reference:%jtalkcom% hello.cs
```

clやvbでは対応する処理で置き換えます。具体的にはffi/clicpp, ffi/csharp, ffi/vbのプログラムのソースを比較してみてください。
コマンドラインでのビルドオプションは、サンプル付属のビルドスクリプトを確認ください。
VisualStudioのIDEを使う場合は、アセンブリの参照にjtalkCOMx64.dllかjtalkCOMx86.dllを追加します。

### COM としての利用

JTalkComはC++/CLIで書かれたマネージDLLですが、COM相互運用機能により、COMのように振る舞います。
このDLLの名前の由来でもあります。
cl.exe, csc.exe, vb.exe でも利用可能ですが、ここではJScriptでの例を示します。
...
hello.wsfという名前でUTF-8エンコードで保存したファイルのアイコンをダブルクリックすれば、しゃべります。
